...
class Reed(JobSite):
    ...
    def get_details(self, job):
        ...
        detail = {
            ...
            "contract": {
                "S": job.find(class_="time").get_text().strip()
                if job.find(class_="time")
                else "null"
            },
            "company": {
                "S": job.find(class_="gtmJobListingPostedBy").get_text().strip()
            },
            "age": {
                "S": job.find(class_="posted-by").get_text().strip().split("by")[0]
            },
...
class Indeed(JobSite):
    ...
    def get_details(obj, job):
        ...
        """
        detail = {
            "id": {"S": job.find(class_="recJobLoc")["id"][10:]},
            "title": {
                "S": job.find(class_="jobtitle turnstileLink").get_text().strip()
            },
            "location": {
                "S": job.find(class_="location accessible-contrast-color-location")
                .get_text()
                .strip()
            },
            "salary": {
                "S": job.find(class_="salaryText").get_text().strip()
                if job.find(class_="salaryText")
                else "null"
            },
            "contract": {"S": "null"},
